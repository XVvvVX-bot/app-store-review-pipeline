...
- Rendered HTML with Playwright remains diagnostic only; repeated scrolling did not reveal more review rows or trigger deeper review network calls. This now holds for both Amazon Shopping and TikTok probes.
...
- Web catalog can exceed the RSS-sized 500-review window: Amazon Shopping depth probes reached 5,042 distinct Postgres reviews through page 253. The page 204-253 continuation inserted 1,000 more unique rows, had 0 final non-200 pages, 0 empty pages, 0 missing text/rating, and stopped at our configured page cap while Apple still returned a next link to offset 5,060. Treat this as a lower-bound proof of depth, not a claim of full historical completeness; retry pressure increases during deep pagination.
- Controlled ingestion now has adaptive RSS-parity stopping: the self-hosted workflow defaults to one app, 35 max web pages, `limit=20`, 5-second request delay, bounded 429 retry/backoff, and `--stop-at-rss-parity`. A Walmart parity smoke moved from 500 web rows vs 556 RSS rows to 560 web rows vs 556 RSS rows, stopping at `target_review_count_reached` after 28 final-200 pages. A pushed-workflow GitHub Actions run on Uber then moved from 0 web rows to 600 web rows vs 587 RSS rows, stopping at `target_review_count_reached` after 30 final-200 pages. This proves the ingestion path can go past the old 25-page / 500-review ceiling when needed without making very deep post-parity pagination routine.
- After the RSS daily workflow advanced Uber's RSS count from 587 to 633, a follow-up web catalog workflow run re-caught parity at 640 web rows vs 633 RSS rows. Later coverage-aware runs selected TikTok, DoorDash, Instagram, YouTube, Netflix, Uber Eats, Life360, SHEIN, Cash App, PayPal, Venmo, ReelShort, Lyft, Freecash, Pokemon GO, and Rips by Triumph automatically. All completed with final status counts of only `200`, 0 final non-200 pages, 0 missing text/rating, and `target_review_count_reached` stops after bounded retry. Current Postgres coverage is still early-stage overall but has crossed the controlled promotion gate: 20 web catalog scopes have data, all 20 are at or above RSS, 0 are below RSS, and 180 RSS scopes have no web catalog rows yet.
- The web catalog schedule now uses coverage-aware `auto` targeting. It chooses the next active app-country scope below RSS parity using the remaining review gap under the configured page cap, and it prioritizes existing below-RSS cleanup scopes before entirely missing scopes. This fixed the earlier behavior where already-started below-RSS scopes could be skipped; Cash App, PayPal, and Venmo were then cleaned up to parity or above.
- The web catalog ingestion history report now includes a Postgres depth section. Current scorecard evidence shows 20 app-country scopes above 500 unique web catalog reviews and Amazon Shopping at 5,042 unique reviews through page 253. Every current tested scope stopped while the terminal page still had a next link, so these are observed lower bounds rather than proven full catalog limits.
- Downloaded workflow artifacts from the recent full single-app ingestion history now pass the controlled-promotion stability gate: 21 clean full single-app runs, 21/21 runs at or above 500 reviews, 13,460 reviews fetched, 673/673 final `200` pages, 0 final non-200 pages, 0 fetch errors, 0 missing text/rating, and 8 retried pages that all recovered. The gate counts both configured-depth completion and `target_review_count_reached` parity stops as successful completion modes.
- The self-hosted web catalog ingestion workflow now respects manual `stop_at_rss_parity=false` dispatches. A previous GitHub Actions expression coerced manual `false` back to the scheduled default `true`, which could make depth probes stop at RSS parity and understate catalog depth.
