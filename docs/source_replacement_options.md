...
- After the RSS daily workflow advanced Uber's RSS count from 587 to 633, a follow-up web catalog workflow run re-caught parity at 640 web rows vs 633 RSS rows. Later coverage-aware runs selected TikTok, DoorDash, and Instagram automatically. TikTok fetched 700 web catalog rows vs 682 RSS rows, DoorDash fetched 660 web catalog rows vs 646 RSS rows, and Instagram fetched 640 web catalog rows vs 638 RSS rows; all stopped at RSS parity with all pages at final `200`. Current Postgres coverage is still early-stage overall: 10 web catalog scopes have data, 7 are at or above RSS, 3 are below RSS, and 190 RSS scopes have no web catalog rows yet. The source is promising, but broad replacement readiness still needs more rotating scheduled evidence.
...
- The web catalog ingestion history report now includes a Postgres depth section. Current depth evidence shows 8 app-country scopes at or above 500 unique web catalog reviews, 7 above 500, and Amazon Shopping at 3,500 unique reviews through page 175. Every current tested scope stopped while the terminal page still had a next link, so these are observed lower bounds rather than proven full catalog limits.
