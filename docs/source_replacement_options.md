...
- After the RSS daily workflow advanced Uber's RSS count from 587 to 633, a follow-up web catalog workflow run re-caught parity at 640 web rows vs 633 RSS rows. Later coverage-aware runs selected TikTok and DoorDash automatically. TikTok fetched 700 web catalog rows vs 682 RSS rows, and DoorDash fetched 660 web catalog rows vs 646 RSS rows; both stopped at RSS parity with all pages at final `200`. Current Postgres coverage is still early-stage overall: 9 web catalog scopes have data, 6 are at or above RSS, 3 are below RSS, and 191 RSS scopes have no web catalog rows yet. The source is promising, but broad replacement readiness still needs more rotating scheduled evidence.
...
- The web catalog ingestion history report now includes a Postgres depth section. Current depth evidence shows 7 app-country scopes at or above 500 unique web catalog reviews, 6 above 500, and Amazon Shopping at 3,500 unique reviews through page 175. Every current tested scope stopped while the terminal page still had a next link, so these are observed lower bounds rather than proven full catalog limits.
