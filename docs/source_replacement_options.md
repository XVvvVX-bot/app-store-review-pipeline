...
- After the RSS daily workflow advanced Uber's RSS count from 587 to 633, a follow-up web catalog workflow run re-caught parity at 640 web rows vs 633 RSS rows. A later coverage-aware run selected TikTok automatically, fetched 700 web catalog rows vs 682 RSS rows, and stopped at RSS parity after 35 final-200 pages with 1 recovered retry. Current Postgres coverage is still early-stage overall: 8 web catalog scopes have data, 5 are at or above RSS, 3 are below RSS, and 192 RSS scopes have no web catalog rows yet. The source is promising, but broad replacement readiness still needs more rotating scheduled evidence.
...
- The web catalog ingestion history report now includes a Postgres depth section. Current depth evidence shows 6 app-country scopes at or above 500 unique web catalog reviews, 5 above 500, and Amazon Shopping at 3,500 unique reviews through page 175. Every current tested scope stopped while the terminal page still had a next link, so these are observed lower bounds rather than proven full catalog limits.
