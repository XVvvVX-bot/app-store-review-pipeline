...
- After the RSS daily workflow advanced Uber's RSS count from 587 to 633, a follow-up web catalog workflow run re-caught parity at 640 web rows vs 633 RSS rows. Later coverage-aware runs selected TikTok, DoorDash, Instagram, YouTube, Netflix, Uber Eats, Life360, and SHEIN automatically. TikTok fetched 700 web catalog rows vs 682 RSS rows, DoorDash fetched 660 vs 646, Instagram fetched 640 vs 638, YouTube fetched 560 vs 560, Netflix fetched 560 vs 556, Uber Eats fetched 560 vs 547, Life360 fetched 560 vs 545, and SHEIN fetched 540 vs 536; all stopped at RSS parity with all pages at final `200`. Current Postgres coverage is still early-stage overall: 15 web catalog scopes have data, 12 are at or above RSS, 3 are below RSS, and 185 RSS scopes have no web catalog rows yet. The source is promising, but broad replacement readiness still needs more rotating scheduled evidence.
- The web catalog schedule now uses coverage-aware `auto` targeting. It chooses the next active app-country scope below RSS parity, preferring reachable gaps under the configured page cap, so scheduled runs should build broad parity evidence faster than simple time-slot rotation.
- The web catalog ingestion history report now includes a Postgres depth section. Current depth evidence shows 13 app-country scopes at or above 500 unique web catalog reviews, 12 above 500, and Amazon Shopping at 5,042 unique reviews through page 253. Every current tested scope stopped while the terminal page still had a next link, so these are observed lower bounds rather than proven full catalog limits.
