...
- After the DoorDash run, current Postgres coverage is 200 RSS scopes, 9 web catalog scopes, 6 web-at-or-above-RSS scopes, 3 web-below-RSS scopes, and 191 RSS scopes without web catalog rows. The next coverage-aware target is Instagram, which has 638 RSS rows and no web catalog rows yet.
- `scripts/summarize_web_catalog_ingestion.py` now adds a Postgres-backed web catalog depth section. The current depth readout shows 9 app-country scopes with web catalog pages, 7 scopes at or above 500 unique reviews, 6 scopes above 500 unique reviews, a maximum of 3,500 unique reviews for Amazon Shopping through page 175, and 9 scopes that stopped while the terminal page still had a next link. Treat these as observed lower bounds, not full historical limits.
