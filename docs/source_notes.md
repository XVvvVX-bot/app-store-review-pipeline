...
- After the Uber Eats, Life360, and SHEIN runs, current Postgres coverage is 200 RSS scopes, 15 web catalog scopes, 12 web-at-or-above-RSS scopes, 3 web-below-RSS scopes, and 185 RSS scopes without web catalog rows. The next coverage-aware target is ReelShort, which has 501 RSS rows and no web catalog rows yet.
- `scripts/summarize_web_catalog_ingestion.py` now adds a Postgres-backed web catalog depth section. The current depth readout shows 15 app-country scopes with web catalog pages, 13 scopes at or above 500 unique reviews, 12 scopes above 500 unique reviews, a maximum of 5,042 unique reviews for Amazon Shopping through page 253, and 15 scopes that stopped while the terminal page still had a next link. Treat these as observed lower bounds, not full historical limits.
