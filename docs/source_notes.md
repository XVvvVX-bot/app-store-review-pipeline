...
- After the Netflix and Amazon Shopping depth runs, current Postgres coverage is 200 RSS scopes, 12 web catalog scopes, 9 web-at-or-above-RSS scopes, 3 web-below-RSS scopes, and 188 RSS scopes without web catalog rows. The next coverage-aware target is Uber Eats, which has 547 RSS rows and no web catalog rows yet.
- `scripts/summarize_web_catalog_ingestion.py` now adds a Postgres-backed web catalog depth section. The current depth readout shows 12 app-country scopes with web catalog pages, 10 scopes at or above 500 unique reviews, 9 scopes above 500 unique reviews, a maximum of 5,042 unique reviews for Amazon Shopping through page 253, and 12 scopes that stopped while the terminal page still had a next link. Treat these as observed lower bounds, not full historical limits.
