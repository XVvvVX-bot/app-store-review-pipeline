...
- Use daily web-only automation only with a known safe pressure state. The scheduled primary workflow now runs daily and reads the safe page cap from Postgres instead of probing every 30 minutes.
...
- Move active pressure discovery into the manual chained `App Store Web Catalog Pressure Search` workflow. The stateful search is stored in `app_store_pressure_state` and starts with one app job, no page cap, a 30-minute per-app-country time budget, 10-second request delay, and no HTTP 429 retries. After clean runs, it increases parallel app jobs one at a time up to four; only after four-way parallelism is clean does it increase the per-app time budget. The first HTTP 429/error triggers a 30-minute cooldown and a same-pressure retry; a second failure rolls back to the previous safe pressure and verifies it after another cooldown.
...
- For the next all-target backfill pass, the workflow uses 200-app rounds with only 5 matrix jobs running at a time: `limit=200`, `max_parallel=5`, `max_pages_per_app_country=0`, `web_time_budget_seconds=1200`, `web_scope_time_budget_seconds=1200`, and `auto_continue=true`. If a full round succeeds, a local-runner continuation job checks Postgres and immediately dispatches the next 200-app round unless every selected app has a `no_next_href` terminal page.
