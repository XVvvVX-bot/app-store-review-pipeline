...
- Use cooldown-aware web-only automation while finding a safe rate. The scheduled primary workflow now runs every 30 minutes at minutes `13` and `43` UTC, avoiding GitHub's high-load `:00`/`:30` schedule slots. It uses a base 5-page cap, an automatic clean-history pressure ramp up to 25 pages, 10-second request delay, 2 HTTP 429 retries, and RSS-parity stopping disabled.
