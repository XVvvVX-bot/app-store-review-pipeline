...
- Large deep backfills can trigger HTTP 429 pressure. The backfill workflow is manually disabled; if deliberately re-enabled for a guarded diagnostic, it handles isolated 429 responses with a 5-minute per-request retry delay plus jitter and relies on the current-run circuit breaker when aggregate pressure becomes unhealthy.
