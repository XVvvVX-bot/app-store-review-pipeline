...
The first full-scope run starts every app at page 1. Existing review keys and trusted overlap make partial writes from interrupted runs safe to revisit. If no app remains backlogged or hard-failed, this run resolves the incident. Otherwise the supervisor places both outcomes in the same bounded, single-app recovery queue and then runs one final 200-scope verification. A hard failure that reaches overlap after an intermediate non-200 response is not trusted: its freshness frontier remains unchanged, and the checkpoint rewinds 25 pages before the first failed page rather than resuming after it. Each targeted run must create a complete one-scope execution with no backlog or hard failure before the app leaves the queue.
