...
Recovery attempt limits are incident-scoped. A successful full-scope verification archives the incident ID and attempt totals in `last_resolved_recovery`, then clears full, verification, and per-app retry counters plus stale run pointers. A later outage therefore starts at `attempt-1`; retry exhaustion from a resolved incident cannot force a new incident directly into `manual_attention`.

The first full-scope run starts every app at page 1. Existing review keys and trusted overlap make partial writes from interrupted runs safe to revisit. If no app remains backlogged or hard-failed, this run resolves the incident. Otherwise the supervisor places both outcomes in the same bounded, single-app recovery queue and then runs one final 200-scope verification. A hard failure that reaches overlap after an intermediate non-200 response is not trusted: its freshness frontier remains unchanged, and recovery rewinds 25 pages before the first failed page rather than resuming after it. For that bounded repair window only, overlap stop is disabled and a page cap ends exactly at the original failed page. The app leaves the repair queue only when a clean execution proves that page was covered; the subsequent uncapped full-scope verification must still reach trusted overlap before the incident resolves.

An infrastructure run that creates no execution, or leaves one `running`/`cancelled`, gets a fresh five-minute stability window and at most three attempts. More than 10 unresolved apps, a monitor-confirmed incomplete execution, a final monitor status that cannot be verified, source-pressure failure, or exhausted retries moves the supervisor to `manual_attention` and stops automatic dispatch. A GitHub workflow may be red during an intermediate recovery pass; the state machine uses persisted scope outcomes and the final Postgres monitor status instead of treating that expected intermediate signal as data loss.

Transient DNS, connect, and timeout failures are retried separately from HTTP 429. Production uses three connection retries with 15-second exponential backoff, a multiplier of 2, and up to 5 seconds of jitter. This provides a roughly two-minute recovery window for a brief host-network interruption while preserving the existing five-minute Apple 429 cooldown. Retry sleeps remain bounded by the overall and per-scope time budgets.
...
The checkpoint query only considers incomplete attempts newer than the scope's last successful catch-up. It chooses the recent attempt that reached the oldest review frontier, then moves 25 pages toward page 1 before resuming. If that attempt contains an intermediate failed page, the rewind anchor is the first failed page rather than the deepest successful page, so recovery cannot skip a hole that occurred before eventual overlap. The safety overlap absorbs normal page drift while trusted review IDs still control the final catch-up stop. After the scope records a clean `caught_up_to_existing_reviews` execution, leave routine scheduled runs in their default page-1 mode.
